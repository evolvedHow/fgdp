#!/usr/bin/env python3
"""
combine_chain_demographics.py — Merge per-chain demographic scores parquets into a
single file with globally unique draw numbers matching the combined plans parquet.

Usage (from fgdp/ root):
    uv run --project fdp python fdp/scripts/combine_chain_demographics.py \\
...
def main() -> None:
    ...
    ap.add_argument("--out-file", default=None,
                    help="Output path (default: {data_dir}/ensemble/{run_name}_demographics.parquet)")
    args = ap.parse_args()
    ...
    out_file = (Path(args.out_file).resolve() if args.out_file
                else ensemble_dir / f"{args.run_name}_demographics.parquet")
    ...
    for i in range(args.n_chains):
        chain_file = ensemble_dir / f"{args.run_name}_chain{i}_demographics.parquet"
        if not chain_file.exists():
            print(f"ERROR: missing {chain_file.name}")
            import sys; sys.exit(1)

        print(f"Loading chain{i} demographics from {chain_file.name}…")
