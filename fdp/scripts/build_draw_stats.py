...
Reads fdp.ensemble_scores for a given plan_id and computes per-draw rollups:
  ...
  - n_competitive_005, n_competitive_007, n_competitive_010 (competitiveness)
...
SELECT
    ...
    COUNT(*) FILTER (
        WHERE dem_2pv IS NOT NULL
          AND ABS(dem_2pv - 0.5) * 2 <= 0.05
    ) AS n_competitive_005,
...
ON CONFLICT (plan_id, draw, year, election_type, office) DO UPDATE SET
    ...
    n_competitive_005 = EXCLUDED.n_competitive_005,
...
def main() -> None:
    ...
    with psycopg.connect(DB_URL) as conn:
        ...
        if row:
            ...
            print(f"  Avg competitive  : {row[6]} districts at 5%  |  {row[7]} at 7%")
            print(f"  Avg |EG|         : {row[8]}")

    # ── Enacted plan summary ─────────────────────────────────────────────────
    print("\nEnacted plan stats (draw=1):")
    with psycopg.connect(DB_URL) as conn:
        rows = conn.execute(_ENACTED_SQL, (plan_id,)).fetchall()
    print(f"  {'Year':<6} {'Office':<12} {'Seats':>8} {'EG%':>7} {'Comp5':>6} {'Comp7':>6} {'Comp10':>7}")
    print("  " + "-" * 56)
    for r in rows:
        print(f"  {r[0]:<6} {r[1]:<12} D{r[2]}/R{r[3]}  {r[4]:>6}%  {r[5]:>5}  {r[6]:>5}  {r[7]:>6}")
